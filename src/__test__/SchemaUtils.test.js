...
describe("(SchemaUtils.test.js)", () => {
  it("validateSchema returns errors for missing schema items", () => {
    expect(SchemaUtils.validateSchema({}, donationSchema).length).toBe(
      donationSchema.required.length
    );
  });

  it("validateSchema returns empty array for valid data", () => {
    expect(SchemaUtils.validateSchema(PAYMENT, donationSchema).length).toBe(0);
  });
