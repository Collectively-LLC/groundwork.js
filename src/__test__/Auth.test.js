...
import { TOKEN, RESPONSE_200, RESPONSE_400 } from "./constants.test";
...
describe("(Auth.test.js)", () => {
  ...
  describe("setters", () => {
    ...
    it("auth should set the tokenType in auth config object", () => {
      ...
      expect(auth.auth).toEqual(
        jasmine.objectContaining({ tokenType: TOKEN.tokenType })
      );
    });

    it("gwid should set the gwid in auth config object", () => {
      auth.gwid = TOKEN.gwid;
      expect(auth.auth).toEqual(jasmine.objectContaining({ gwid: TOKEN.gwid }));
    });
  });
  ...
  it("fetchUsingPassword should get a TOKEN back on success", done => {
    ...
    auth
      .fetchUsingPassword("foo@mail.com", "abc123")
      .then(resp => {
        response = resp;
      })
      .catch(err => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        expect(response.data).toEqual(
          jasmine.objectContaining(RESPONSE_200.responseText)
        );
        expect(response.status).toEqual(200);
        expect(response.statusText).toEqual("OK");
        expect(response.headers["content-type"]).toEqual("application/json");
        // Ensure config was updated
        expect(auth.config.get("auth")).toEqual(
  ...
  it("destroy token prevents additional authed reqs", done => {
    ...
    setTimeout(() => {
      ...
      expect(requestA.requestHeaders).toEqual(
        jasmine.objectContaining({
      ...
      setTimeout(() => {
        ...
        expect(requestB.requestHeaders).not.toEqual(
          jasmine.objectContaining({
            Authorization: `${TOKEN.tokenType} ${TOKEN.accessToken}`
          })
        );
